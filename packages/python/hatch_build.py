...
# Membership rather than `==` so an unrecognised build version still gets the
# asset: shipping a distribution without the viewer is the failure that matters.
EDITABLE_VERSIONS = frozenset({"editable"})
...
class ViewerAssetBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:
        if version in EDITABLE_VERSIONS:
